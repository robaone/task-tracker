...
1. **Task reading** — language-agnostic abstraction for reading tasks across backends (Jira, GitHub Issues, Linear, markdown files, etc.) with a common read-only interface.
...
**Read-only by design.** This CLI does not create, update, or delete tasks — that responsibility stays with each backend's native tooling (Jira UI, GitHub CLI, etc.). The CLI is a consumer of task data for code review context.
...
interface Task {
  ...
  url?: string       // link back to the native task manager
...
No create/update DTOs — the CLI never writes tasks.
...
A `TaskProvider` interface that every backend implements. This is the contract for reading task data and retrieving review context.
...
The JSON backend reads from a file; it does not create, update, or delete tasks. Tasks are managed by whatever external tool writes to that file.
...
task-tracker init                      # configure backend (no task data created)
task-tracker download <id> [-o file]   # export task to JSON file
```
...
task-tracker/
├── src/
│   ├── types.ts            # Core types (no DTOs)
│   ├── provider.ts         # TaskProvider interface (read-only)
│   ├── registry.ts         # Provider registry
│   ├── providers/
│   │   └── json.ts         # Local JSON backend (read-only)
...
2. **Types** — `src/types.ts` (Task + context types, no DTOs)
3. **Interface** — `src/provider.ts` (read-only + context methods)
4. **JSON backend** — `src/providers/json.ts` + tests (read-only)
5. **Registry** — `src/registry.ts`
6. **Diff module** — `src/diff.ts` + tests
7. **Bundler** — `src/bundler.ts`
8. **CLI** — `src/cli.ts` (read-only + review-prep commands)
...
2. Implement read-only methods (`list`, `get`, `search`) + context methods
3. Write tests
4. Register it in the registry
5. Done — CLI and library consumers get everything for free

Since the interface is read-only, every backend is expected to support all methods (though context methods like `getAttachments` may return `[]` if not applicable). No `NotSupportedError` is needed — the interface simply doesn't include write operations.
